...
myDog.name='Fido';
// Change myOtherDog type to hairless using square bracket notation
var myOtherDog = {
	type: 'poodle',
	name: 'Fluffy'
};
...
for (var x in briefing) {
	if(briefing.hasOwnProperty('secret')) {
		briefing[x] = 'redacted';
		}
}
...
// function removeThings(numbers, string) {
// 	for (var s in numbers){
// 	if(numbers.hasOwnProperty(s)) {
// 		delete s;
// 		}
// 	return numbers;
// }
...
function getPropertyValue (obj,str) {
  return obj.getOwnPropertyNames(str);
}
...
function spiderTest(actor) {
  if(name = 'Andrew Garfield'){
    return 10;}
  if(name = 'Tobey Maguire'){
    return 0;}
  else {
    return 'not a spiderman';}
}
...
function whatIsItOutside (temperature, humidity, cloudiness){
  ...
  if(temp > 85, humidty < 45, cloudiness < 20){
    return "I'm literally in the desert"
  }
  if(temp>90 || humidty > 55 || cloudiness > 65){
